...
from utils import train, test, contrast_train
...
def get_args():
    ...
    parser.add_argument('--video_embed_dim', default=512, type=int)
    ...
    parser.add_argument('--contrast_num_epochs', default=20, type=int, help='the epoch of train')
    parser.add_argument('--batch_size', default=32, type=int, help='the batch size of dataset')
    parser.add_argument('--contrast_batch_size', default=100, type=int, help='the batch size of dataset')
    parser.add_argument('--lr', default=1e-5, type=float, help='the learning rate of bert')
    parser.add_argument('--contrast_lr', default=2e-5, type=float, help='the learning rate of bert')
...
def main():
    ...
    if args.two_roles:
        save_dir = f'checkpoint/{args.mode}_{args.max_turns}_{args.alpha}_two_roles'
    else:
        save_dir = f'checkpoint/{args.mode}_{args.max_turns}_{args.alpha}'
    ...
    if args.two_roles:
        output_dir = os.path.join(args.output_dir, args.mode + f'_{args.max_turns}_{args.alpha}' + '_two_roles')
    else:
        output_dir = os.path.join(args.output_dir, args.mode + f'_{args.max_turns}_{args.alpha}')
    ...
    if args.do_train:
        ...
        contrast_train_dataset = DialogContrastWithGenerationDataset(args, 'train')
    ...
    if args.do_train:
        ...
        contrast_train_loader = DataLoader(dataset=contrast_train_dataset,
                                batch_size=args.contrast_batch_size,
                                shuffle=True,
                                collate_fn=contrast_train_dataset.collate_fn
                                )
    ...
    if args.do_train:
        num_training_steps = len(contrast_train_loader) * args.contrast_num_epochs
        
        optimizer = torch.optim.AdamW(filter(lambda p: p.requires_grad, model.parameters()),
                                    lr=args.contrast_lr, weight_decay=args.weight_decay)
        scheduler = transformers.get_cosine_schedule_with_warmup(optimizer,
                                                                num_warmup_steps=args.warm_up * num_training_steps,
                                                                num_training_steps=num_training_steps)
        for i in range(args.contrast_num_epochs):
            logging.info(f"epoch {i+1}:")
            contrast_train(model, optimizer, scheduler, contrast_train_loader, args)
